...
if [ -r ${CONFIGFILE} ]; then
	...
	BACKUPDIR="/home/db/mysql-backups/"
	...
	MAILADDR="kotowicz@ini.uzh.ch,michael@ini.uzh.ch"
...
then
	...
	then
		...
		mutt -s "${ERRORNOTE} MySQL Backup Log and SQL Files for ${HOST} - ${DATE}" -a ${BACKUPFILES} -c ${MAILADDR} < ${LOGFILE}		#send via mutt
	else
