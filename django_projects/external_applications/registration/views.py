...
def notify(account):
    ...
    subject = 'New account activated on %s: %s for %s %s' % (site_name, account.email, account.first_name, account.last_name)
    user_list = [u.email for u in 
                User.objects.filter(is_staff=True, is_superuser=True).exclude(email="").order_by('id')]

    message = 'New user account "%s" for user "%s" (with email address "%s") has been activated.' % (account.username, account.get_full_name(), account.email)
    this_email = EmailMessage(subject, message, to=[user_list[0]])
    this_email.send()
